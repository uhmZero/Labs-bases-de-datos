...
INSERT INTO usuario (nombre, apellido, email, contraseña, fecha_registro, ubicacion, fecha_nacimiento, alergias, tipo_sangre)
VALUES
    ('Carlos',  'Méndez',    'carlos.mendez@tele.cr',    'hash_cm1', '2024-01-10', 'San José, CR',  '1990-05-15', 'Penicilina', 'O+'),  -- id 1
    ('Ana',     'Vargas',    'ana.vargas@tele.cr',        'hash_av2', '2024-02-20', 'Heredia, CR',   '1985-11-22', 'Ninguna',    'A-'),  -- id 2
    ('Luis',    'Fernández', 'luis.fernandez@tele.cr',    'hash_lf3', '2023-06-01', 'Cartago, CR',   '1975-03-08', 'Ninguna',    'B+'),  -- id 3 (especialista)
    ('María',   'Solís',     'maria.solis@tele.cr',       'hash_ms4', '2024-03-01', 'Alajuela, CR',  '1992-07-30', 'Latex',      'AB+'); -- id 4
...
INSERT INTO paciente (id_paciente) VALUES (1), (2), (4);
...
-- Foro vinculado al plan 1 (moderado por el especialista)
INSERT INTO foro (id_especialista, id_plan_tra, nombre, patologia, fecha_creacion) VALUES
    (3, 1, 'Foro de Rehabilitación de Rodilla', 'Lesión ligamentaria', '2024-01-05');  -- id 1
...
INSERT INTO publicacion (id_foro, id_paciente, contenido, fecha, autor) VALUES
    (1, 1, 'El plan mejoró bastante mi movilidad después del tercer ingreso.', '2024-03-05', 'Carlos Méndez'),
    (1, 1, 'Los ejercicios de la Fase de Ataque son muy efectivos, los recomiendo.', '2024-03-10', 'Carlos Méndez');
...
JOIN      especialista      esp   ON esp.id_especialista   = ip.id_especialista
JOIN      usuario           u_esp ON u_esp.id_usuario      = esp.id_especialista
LEFT JOIN foro              f     ON f.id_plan_tra         = reincid.id_plan_tra
LEFT JOIN publicacion       pub   ON pub.id_foro           = f.id_foro
                                 AND pub.id_paciente       = reincid.id_paciente
